...
def _get_data(path, scraper_name, project_id):
    """Private function to get the absolute path to the downloaded file."""
    cwd = os.path.abspath(os.path.dirname(__file__))
    return os.path.join(cwd, 'pdf', scraper_name, project_id, path)


def check_make_directory(scraper_name, project_id):
    """Check to see if the `pdf/$PROJECT_ID` dir exists. If not, make it."""
    cwd = os.path.abspath(os.path.dirname(__file__))
    if not os.path.exists(os.path.join(cwd, 'pdf', scraper_name, project_id)):
        os.makedirs(os.path.join(cwd, 'pdf', scraper_name, project_id))
...
def download_project_documents(url, scraper_name, project_id):
    """
    ...
    local_filename = _get_data(filename, scraper_name, project_id)
